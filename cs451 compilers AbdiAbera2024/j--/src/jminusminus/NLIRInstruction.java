...
abstract class NLIRInstruction {
    /**
     * Maps JVM opcode to a string mnemonic for LIR instructions. For example, imul is mapped to
     * the "MUL".
     */
    ...
    /**
     * Replace references to virtual registers in this LIR instruction with references to
     * physical registers.
     */
...
class NLIRArithmetic extends NLIRInstruction {
     ...
     */
    public NLIRArithmetic(NBasicBlock block, int id, int opcode, NLIRInstruction lhs,
                          NLIRInstruction rhs) {
        super(block, id);
        this.opcode = opcode;
        reads.add(lhs.write);
     ...
     */
    public void allocatePhysicalRegisters() {
        NInterval input1 = block.cfg.intervals.get(reads.get(0).number()).childAt(id);
        NInterval input2 = block.cfg.intervals.get(reads.get(1).number()).childAt(id);
     ...
     */
    public String toString() {
        return id + ": " + lirMnemonic[opcode] + " " + reads.get(0) + " " + reads.get(1) +
                " " + write;
    }
}
...
class NLIRStringConstant extends NLIRInstruction {
    ...
    // For generating unique label for the string in the data segment.
    ...
    public NLIRStringConstant(NBasicBlock block, int id, String value) {
        super(block, id);
        this.value = value;
        write = new NVirtualRegister(NControlFlowGraph.regId++, "L", "Ljava/lang/String;");
        block.cfg.registers.add((NVirtualRegister) write);
        labelSuffix = 0;
    }

    /**
     * Creates and returns a label for LIR code.
     *
     * @return the label for LIR code.
     */
    ...
    public void toSpim(PrintWriter out) {
        ...
        s += "    .word " + value.length() + " # String length (not including null terminator)\n";
        s += "    .asciiz \"" + value + "\" # String terminated by null character 0\n";
...
class NLIRConditionalJump extends NLIRInstruction {
    ...
    private NBasicBlock onTrueDestination;

    // Block to jump to on false.
    private NBasicBlock onFalseDestination;
...
class NLIRInvoke extends NLIRInstruction {
     ...
     */
    public NLIRInvoke(NBasicBlock block, int id, int opcode, String target, String name,
                      ArrayList<NRegister> arguments, String sType, String lType) {
        super(block, id);
        this.opcode = opcode;
        this.target = target;
        this.name = name;
    ...
    public void allocatePhysicalRegisters() {
        for (int i = 0; i < reads.size(); i++) {
            NInterval input = block.cfg.intervals.get(reads.get(i).number()).childAt(id);
    ...
    public void toSpim(PrintWriter out) {
        out.printf("    jal %s.%s\n", target.replace("/", "."),
                name.equals("<init>") ? "__init__" : name);
    }
...
class NLIRPutField extends NLIRInstruction {
     ...
     */
    public NLIRPutField(NBasicBlock block, int id, int opcode, String target, String name,
                        String sType, String lType, NLIRInstruction value) {
        super(block, id);
        this.opcode = opcode;
        this.target = target;
        this.name = name;
     ...
     */
    public String toString() {
        return id + ": " + lirMnemonic[opcode] + " " + target + "." + name + " = " + reads.get(0);
    }
}
...
class NLIRGetField extends NLIRInstruction {
     ...
     * @param lType  type (long name) of the field.
     */
    public NLIRGetField(NBasicBlock block, int id, int opcode, String target, String name,
     ...
     */
    public String toString() {
        return id + ": " + lirMnemonic[opcode] + " " + write + " = " + target + "." + name;
    }
}
...
class NLIRNewArray extends NLIRInstruction {
     ...
     */
    public NLIRNewArray(NBasicBlock block, int id, int opcode, int dim, String sType,
                        String lType) {
        super(block, id);
        this.opcode = opcode;
        this.dim = dim;
...
class NLIRALoad extends NLIRInstruction {
    ...
    public String toString() {
        return id + ": " + lirMnemonic[opcode] + " " + write + "= " + reads.get(0) + "[" +
                reads.get(1) + "]";
    }
}
...
class NLIRAStore extends NLIRInstruction {
     ...
     * @param lType    type (long name) of the array.
     */
    public NLIRAStore(NBasicBlock block, int id, int opcode, NLIRInstruction arrayRef,
                      NLIRInstruction index, NLIRInstruction value, String sType, String lType) {
        super(block, id);
        this.opcode = opcode;
        reads.add(arrayRef.write);
        reads.add(index.write);
...
class NLIRMove extends NLIRInstruction {
     ...
     */
    public void allocatePhysicalRegisters() {
        NInterval input = block.cfg.intervals.get(reads.get(0).number()).childAt(id);
        NInterval output = block.cfg.intervals.get(write.number()).childAt(id);
...
class NLIRLoadLocal extends NLIRInstruction {
    ...
    public NLIRLoadLocal(NBasicBlock block, int id, int local, String sType, String lType) {
        ...
        if (local < 4) {
            ...
            block.cfg.registers.set(A0 + local, NPhysicalRegister.regInfo[A0 + local]);
        } else {
            write = new NVirtualRegister(NControlFlowGraph.regId++, sType, lType);
            block.cfg.registers.add((NVirtualRegister) write);
        }
...
class NLIRLoad extends NLIRInstruction {
     ...
     * @param offsetFrom whether offset relative to stack pointer (sp) or frame pointer (fp).
     * @param register   register to load to.
     */
    public NLIRLoad(NBasicBlock block, int id, int offset, OffsetFrom offsetFrom,
                    NRegister register) {
        super(block, id);
        this.offset = offset;
     ...
     */
    public String toString() {
        return id + ": LOAD " + (offsetFrom == OffsetFrom.FP ? "[frame:" : "[stack:") +
                offset + "] " + register;
    }
}
...
class NLIRStore extends NLIRInstruction {
     ...
     */
    public NLIRStore(NBasicBlock block, int id, int offset, OffsetFrom offsetFrom,
                     NRegister register) {
        super(block, id);
        this.offset = offset;
        this.offsetFrom = offsetFrom;
     ...
     */
    public String toString() {
        return id + ": STORE " + reads.get(0) + " " + (offsetFrom == OffsetFrom.FP ? "[frame:" :
                "[stack:") + offset + "]";
    }
}
