...
public class Parser {
    ...
    private JClassDeclaration classDeclaration(ArrayList<String> mods) {
        ...
        Type superC;
        if (have(EXTENDS)) {
            superC = qualifiedIdentifier();
        } else {
            superC = Type.OBJECT;
        }
        ArrayList<TypeName> ident = null;
        if (have(IMPLEMENTS)) {
            ident = new ArrayList<TypeName>();
            do {
                ident.add(qualifiedIdentifier());
            }
            while (have(COMMA));

        }
        return new JClassDeclaration(line, mods, name, superC, ident, classBody());
    }
    //this parses an interface declaration and retuns AST
    ...
    private JInterfaceDeclaration interfaceDeclaration(ArrayList<String> mods) {
        ...
        mustBe(IDENTIFIER);
        String name = scanner.previousToken().image();
        ArrayList<TypeName> superInter = null;
        if (have(EXTENDS)) {
            superInter = new ArrayList<TypeName>();
            do {
                superInter.add(qualifiedIdentifier());
            }
            while (have(COMMA));
        }
        return new JInterfaceDeclaration(line, mods, name, superInter, interfaceBody());
    }


    //this place parses body and returns list of memebers in a body
    //interfaceBody ::= LCURLY

    private ArrayList<JMember> interfaceBody() {
        ArrayList<JMember> memberof = new ArrayList<JMember>();
        mustBe(LCURLY);
        while (!see(RCURLY) && !see(EOF)) {
            ArrayList<String> mod = modifiers();
            memberof.add(interfaceMemberDecl(mod));
        }
        mustBe(RCURLY);
        return memberof;
    ...
    /*
    this parses a interface and returns AST
    interfacememberDecl  ::= (VOID | type ) IDENTIFIER fromalParemeters
    THROWS qualifiedidentifier { COMMA Type vairable

     */
    //question 10
    private JMember interfaceMemberDecl(ArrayList<String> mods) {
        int line = scanner.token().line();
        JMember interfaceMeDecl = null;
        Type type = null;
        if (have(VOID)) {
            ...
            mustBe(IDENTIFIER);
            String name = scanner.previousToken().image();
            ArrayList<JFormalParameter> param = formalParameters();
            ArrayList<TypeName> exceptionss = null;
            if (have(THROWS)) {
                exceptionss = new ArrayList<TypeName>();
                do {
                    exceptionss.add(qualifiedIdentifier());
                }
                while (have(COMMA));
            }
            mustBe(SEMI);
            interfaceMeDecl = new JMethodDeclaration(line, mods, name, type, param, exceptionss, null);

        } else {
            type = type();
            //this is for non void
            if (seeIdentLParen()) {
                if (!mods.contains("abstract")) {
                    mods.add("abstract");
                }
                ...
                mustBe(SEMI);
                interfaceMeDecl = new JMethodDeclaration(line, mods, name, type, params, exceptions, null);
                //this  is the field
            } else {
                mustBe(SEMI);
                interfaceMeDecl = new JFieldDeclaration(line, mods, variableDeclarators(type));

            }
        }
        return interfaceMeDecl;
    }
    ...
    private JMember memberDecl(ArrayList<String> mods) {
        ...
        //this is to check the token represents
        if (seeIdentLParen()) {
            mustBe(IDENTIFIER);
            String name = scanner.previousToken().image();
            ArrayList<JFormalParameter> params = formalParameters();
            ArrayList<TypeName> exceptionss = null;
            //this 'throws' keyword is present wich shows in exception in the constructor
            if (have(THROWS)) {
                exceptionss = new ArrayList<TypeName>();
                do {
                    exceptionss.add(qualifiedIdentifier());
            ...
            //this is parsing the block
            JBlock body = block();
            memberDecl = new JConstructorDeclaration(line, mods, name, params, exceptionss, body);
        } else {
            Type type = null;
            //checking if VOID keyword is represnts
            if (have(VOID)) {
                // A void method.
                type = Type.VOID;
                mustBe(IDENTIFIER);
                ...
                memberDecl = new JMethodDeclaration(line, mods, name, type, params, exceptionss, body);
            } else {
                type = type();
                if (seeIdentLParen()) {
                    // A non void method.
    ...
    private JStatement statement() {
        int line = scanner.token().line();
        // this to check the current token reperesnts '{'
        if (see(LCURLY)) {
            return block();
        } else if (have(IF)) {
            ...
            ArrayList<JStatement> forInitial = null;
            ArrayList<JStatement> forupdate = null;
            JExpression forCondition = null;
            JStatement forBody;
            mustBe(LPAREN);
            //this to check the initial part of 'for' statment
            if (!see(SEMI))
                forInitial = forInitial();
            mustBe(SEMI);
            if (!see(SEMI))
            ...
            mustBe(RPAREN);
            forBody = statement();
            return new JForStatement(line, forInitial, forCondition, forupdate, forBody);
            ...
            //handles Continue statment
            mustBe(SEMI);
            return new JContinueStatement(line);
        } else if (have(SWITCH)) {
            ArrayList<SwitchStatementGroup> switchBStatementGroups = new ArrayList<>();
            JExpression parExpre = parExpression();
            mustBe(LCURLY);
            while (!see(RCURLY) && !see(EOF)) {
                switchBStatementGroups.add(switchBlockStatementGroup());
            }
            mustBe(RCURLY);
            return new JSwitchStatement(line, parExpre, switchBStatementGroups);

        } else if (have(TRY)) {
            JBlock tryBlock = block();
            ArrayList<JFormalParameter> catchFromalParamterof = new ArrayList<>();
            ArrayList<JBlock> catchBloc = new ArrayList<>();
            JBlock finalBlock = null;
            while (have(CATCH)) {
                mustBe(LPAREN);
                catchFromalParamterof.add(formalParameter());
                mustBe(RPAREN);
                catchBloc.add(block());
            }
            if (have(FINALLY)) {
            ...
            return new JTryStatement(line, tryBlock, catchFromalParamterof, catchBloc, finalBlock);
        } else if (have(THROW)) {
    ...
    /*
    this parses and retunrs forInitial
    * forIniatial ::= statementExpression { COMMA statementExoression }
    retuns list of formal parameters
    * */
    private ArrayList<JStatement> forInitial() {
        ArrayList<JStatement> res = new ArrayList<>();
        if (seeBasicType() || seeReferenceType()) {
            int line = scanner.token().line();
            res.add(new JVariableDeclaration(line, variableDeclarators(type())));
        } else {
            res.add(statementExpression());
            while (have(COMMA)) {
                res.add(statementExpression());
            }
        }
   ...
   this parses and retunrs forUpdate
   * forIniatial ::= statementExpression { COMMA statementExoression }
   retuns list of formal parameters
   ...
   * */
    ...
    /*
    this area used to parse a switch block statemnts also it retuns AST
    switchBlockStatementGroup ::= switchL { switchL } {blockst }
  * @return AST for swichblockstaments.
    * */
    private SwitchStatementGroup switchBlockStatementGroup() {
        int line = scanner.token().line();
        ArrayList<JExpression> switchL = new ArrayList<JExpression>();
        ArrayList<JStatement> blockSt = new ArrayList<JStatement>();
        //this will be the first switchL
        switchL.add(switchLabel());
        //when 0 or more followign labels
        while (see(CASE) || see(DEFAULT)) {
            switchL.add(switchLabel());
        }
        while (!see(CASE) && !see(RCURLY) && !see(DEFAULT)) {
            blockSt.add(blockStatement());
        }
        return new SwitchStatementGroup(switchL, blockSt);
    }
    ...
    /*
    this area parses switch label expression and returns AST
    switchLabel ::= CASE ecpression COLON | DEFUALT COLON
    * @return AST for swichblockstaments.
    * */
    ...
    private Type basicType() {
        if (have(BOOLEAN)) {
            ...
            return Type.INT;
            //if it have LONG then return LONG
        } else if (have(LONG)) {
            return Type.LONG;
            //if it have DOUBLE then return DOUBLE
        } else if (have(DOUBLE)) {
            return Type.DOUBLE;
        } else {
     ...
     *
     * <pre>
     *  zndExpression ::= equalityExpression { LAND equalityExpression }
     * </pre>
     *
    ...
    private JExpression andExpression() {
        ...
        while (more) {
            if (have(AND)) {
                ...
                lhs = new JOrOp(line, lhs, conditionalAndExpression());
            } else if (have(XOR)) {
                lhs = new JXorOp(line, lhs, conditionalAndExpression());
            } else if (have(LOR)) {
    ...
    private JExpression conditionalAndExpression() {
        ...
        while (more) {
            if (have(LAND)) {
                ...
                lhs = new JAndOp(line, lhs, inclusiveOrExpression());
            } else if (have(OR)) {
                lhs = new JOrOp(line, lhs, conditionalAndExpression());
            } else if (have(XOR)) {
                lhs = new JXorOp(line, lhs, conditionalAndExpression());
 ...
 /*
    private JExpression unaryExpression() {
        int line = scanner.token().line();
        if (have(INC)) {
            return new JPostIncrementOp(line, unaryExpression());
        } else if (have(MINUS)) {
            return new JNegateOp(line, unaryExpression());
        } else if (have(NOT)) {
 ...
 */
    ...
    this parses conditional-or also returns an AST
    conditionalOrExpre ::= conditionalOrExpre { LOR conditionalAndExprestion
    returns AST condotionalOr expression
    * */
    ...
    /*
    this parses conditional expression and return AST

   conditonlaExpression ::= conditonlaExpression [ QUESTION and COLON conditonlaExpression ]
   returns an AST
    * */
    private JExpression conditionalExpression() {
        ...
        //while (m) {
        if (have(QUESTION)) {
            JExpression tBranch = expression();
            mustBe(COLON);
            return new JConditionalExpression(line, lhs, tBranch, conditionalExpression());
        } else {
            return lhs;
        }
        //}
     ...
     *   CHAR_LITERAL, DOUBLE_LITERAL, LONG_LITERAL
     * </pre>
     *
...
//My homework was based on what we learned in class, and I modified Parser.
//https://swamiiyer.net/cs451/course_info.html#cs_account
