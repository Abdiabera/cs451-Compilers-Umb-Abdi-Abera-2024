...
class JGreaterEqualOp extends JComparisonExpression {
    ...
    public JExpression analyze(Context context) {
        ...
        if (!lhs.type().equals(Type.INT) && !lhs.type().equals(Type.LONG) && !lhs.type().equals(Type.DOUBLE)) {
            JAST.compilationUnit.reportSemanticError(line(), "Invalid left operand type for " + operator);
        }
        if (!rhs.type().equals(lhs.type())) {
            JAST.compilationUnit.reportSemanticError(line(), "Right operand type must match left operand type for " + operator);
        }
        type = Type.BOOLEAN;
        return this;
    }
